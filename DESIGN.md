...
| 2026-09-22 | `line` (dark) lightened `#383b40` → `#40434a` | Direct user report: the dividers read too dark against `bg` (`#1a1c1f`) on an actual dark-mode screen. A modest lighten, not a re-derivation — dark theme's broader "not yet visually verified" caveat still stands for every other token. |
